...
const onBeforeRequest = (d,n) => {
  let hostname = toHostname(d.url); 
  if (prefs.map[n]) {
    hostname = n;
  }
  if (once.length) {
  ...
  if (prefs.map[hostname]) {
    ...
    else if (prefs.map[hostname].startsWith('{') && prefs.map[hostname].endsWith('}')) {
      let addprefs = JSON.parse(prefs.map[hostname]);
      if (addprefs.ignore)
      {
        for (const ignore of addprefs.ignore) {
          if (d.url.indexOf(ignore) !== -1)
          {
            return {}
          }
        }
      }
      if (addprefs.quarantine && d.requestId)
      {
        let quarantineId = hostname;
        if (addprefs.quarantine.id)
        {
          quarantineId = addprefs.quarantine.id;
        }
        if (addprefs.quarantine.time)
        {
          if (typeof(quarantine[quarantineId]) != 'undefined' && quarantine[quarantineId] && quarantine[quarantineId].time)
          {
            if (quarantine[quarantineId].time + (addprefs.quarantine.time*1000) > Date.now())
            {
              return {};
            }
            quarantine[quarantineId].time = Date.now();
            quarantine[quarantineId].ttime = Date.now();
          }
          else
          {
            quarantine[quarantineId] = new Object();
            quarantine[quarantineId].time = Date.now();
            quarantine[quarantineId].ttime =Date.now();
          }
        }
        if (addprefs.quarantine.number)
        {
          if (typeof(quarantine[quarantineId]) != 'undefined' && quarantine[quarantineId] && quarantine[quarantineId].number)
          {
            if (quarantine[quarantineId].number >= addprefs.quarantine.number)
            {
              return {};
            }
            quarantine[quarantineId].number ++ ;
          }
          else
          {
            quarantine[quarantineId] = new Object();
            quarantine[quarantineId].number = 1;
          }
        }
      }
      if (addprefs.text && d.requestId)
      {
        let filter = chrome.webRequest.filterResponseData(d.requestId);
        let decoder = new TextDecoder("utf-8");
        let encoder = new TextEncoder();
        filter.onstop = event =>
        {
          filter.write(encoder.encode(addprefs.text));
          filter.disconnect();
        };
        return {}
      }
      else if (addprefs.url && d.requestId)
      {
        return {
          'redirectUrl': addprefs.url + (search || '')
        };
      }
      else if (addprefs.url)
      {
        return {
          'redirectUrl': addprefs.url + (search || '')
        };
...
const onBeforeHeaders = (d,n) => {
  ...
  // redirect
  if (prefs.map[hostname]) {
    if (prefs.map[hostname].startsWith('{') && prefs.map[hostname].endsWith('}')) {
      let addprefs = JSON.parse(prefs.map[hostname]);
      if (addprefs.headers)
      {
        if (addprefs.quarantine)
        {
          let quarantineId = hostname;
          if (addprefs.quarantine.id)
          {
          ...
          if (addprefs.quarantine.htime)
          {
            if (typeof(quarantine[quarantineId]) != 'undefined' && quarantine[quarantineId] && quarantine[quarantineId].ttime)
            {
              if (quarantine[quarantineId].ttime + (addprefs.quarantine.htime*1000) < Date.now())
              {
                return {};
              }
              quarantine[quarantineId].ttime = 0;
            }
            else
            {
              if (quarantine[quarantineId].ttime==0)
              {
                return {};
              }
            }
        ...
        for (let i = 0; i < d.requestHeaders.length; ++i) {
          for (const header of addprefs.headers) {
            if (d.requestHeaders[i].name === header[0]) {
              d.requestHeaders[i].value = header[1];
            }
          }
        }
...
const onBeforeRequestDirect = d => {
  for (var i = 0; i < directPattern.length ; i++) {
    let rule = directPattern[i];
    if (rule.test(d.url)) {
      return onBeforeRequest(d,i);
    }
  }
};
const onBeforeHeadersDirect = d => {
  for (var i = 0; i < directPattern.length ; i++) {
    let rule = directPattern[i];
    if (rule.test(d.url)) {
      return onBeforeHeaders(d,i);
    }
  }
};
...
const observe = () => {
  if (prefs.blocked.length && prefs.reverse === false) {
    ...
    chrome.webRequest.onBeforeSendHeaders.addListener(onBeforeHeadersDirect, {
      'urls': ['*://*/*'],
      'types': ['main_frame', 'sub_frame']
...
chrome.storage.local.get(prefs, p => {
  const next = () => {
    ...
    if (prefs['rules-db']) {
      fetch(prefs['rules-db']).then(res => res.json()).then((out) => {
        Object.assign(prefs, out);
        schedule.build();
        observe();
        contextmenu.build();
        prefs.blocked=[];
...
const onMessage = (request, sender, response) => {
  ...
  else if (request.method === 'append-to-prefs') {
    if (!prefsd) {
      prefsd=request.data;
